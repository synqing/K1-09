...
void precompute_goertzel_constants() {
  for (uint16_t i = 0; i < NUM_FREQS; i++) {
    int16_t n = i;
    // MODIFICATION [2025-09-20 22:50] - CRITICAL-FIX-001: Prevent array bounds violation
    // FAULT DETECTED: notes[n + CONFIG.NOTE_OFFSET] can access beyond 64-entry array with offset > 0
    // ROOT CAUSE: 64-bin reversion didn't account for NOTE_OFFSET access pattern in system.h
    // SOLUTION RATIONALE: Clamp array access to valid range to prevent undefined behavior
    // IMPACT ASSESSMENT: Prevents Goertzel coefficient corruption that causes spectral distortion
    // VALIDATION METHOD: System should no longer exhibit "compressed dog shit" audio behavior
    // ROLLBACK PROCEDURE: Remove bounds check if frequency mapping issues arise

    int16_t note_index = n + CONFIG.NOTE_OFFSET;
    // Clamp to valid array bounds (0 to 63 for 64-entry array)
    if (note_index >= 64) note_index = 63;
    ...
    // Helper lambda to safely access notes array with bounds checking
    auto safe_notes_access = [](int16_t index) -> float {
      if (index >= 64) return notes[63]; // Clamp to last entry
      if (index < 0) return notes[0];    // Clamp to first entry
      return notes[index];
    };

    if (i == 0) {
      neighbor_left = safe_notes_access(n + CONFIG.NOTE_OFFSET);
      neighbor_right = safe_notes_access(n + CONFIG.NOTE_OFFSET + 1);
    } else if (i == NUM_FREQS - 1) {
      neighbor_left = safe_notes_access(n + CONFIG.NOTE_OFFSET - 1);
      neighbor_right = safe_notes_access(n + CONFIG.NOTE_OFFSET);
    } else {
      neighbor_left = safe_notes_access(n + CONFIG.NOTE_OFFSET - 1);
      neighbor_right = safe_notes_access(n + CONFIG.NOTE_OFFSET + 1);
    }
