...
void track_audio_metrics(float* magnitudes, uint8_t num_bins) {
    ...
    // Use debug manager for proper timing control (2.8 second interval with stagger)
    if (debug_counter % 200 == 0) {
        DBG_FREQ_SPECTRUM(peak, frequencies[peak].target_freq, max_val,
        ...
        // MODIFICATION [2025-09-20 22:30] - BIN-REVERT-96-64-001: High frequency band adjustment
        // FAULT DETECTED: Loop range (64 to NUM_FREQS) will never execute when NUM_FREQS=64
        // ROOT CAUSE: Hardcoded frequency band boundaries based on 96-bin assumption
        // SOLUTION RATIONALE: Adjust high band to 43-63 for balanced 64-bin distribution
        // IMPACT ASSESSMENT: Maintains frequency band monitoring with proper coverage
        // VALIDATION METHOD: Verify high frequency band data is logged correctly
        // ROLLBACK PROCEDURE: Restore original 64-96 range if frequency analysis issues arise

        // High: 43-63 (554-830Hz for 64-bin configuration)
        for (uint8_t i = 43; i < NUM_FREQS; i++) {
            high_sum += magnitudes[i];
            if (magnitudes[i] > 1.0) high_active++;
        }
        
        DBG_FREQ_DIST(low_active, low_sum, mid_active, mid_sum, high_active, high_sum);
