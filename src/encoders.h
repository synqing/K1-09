...
#include "palettes/palette_luts_api.h" // Palette count + LUT accessors
...
void check_encoders(uint32_t t_now) {
    ...
    auto debugEncoder = [&](uint8_t channel, int32_t value, const char* name, ConfigFixed new_value) {
        if (debug_mode && value != 0 && (t_now - last_debug_time) > 100) {
            USBSerial.print("\033[36m[DBG E");
            USBSerial.print(channel);
            USBSerial.print("]\033[0m Raw: ");
    ...
    if (chroma_rel != 0) {
        ...
        if (new_value != current_val) {
            // CHROMA BLOCKING [2025-09-20] - Prevent CHROMA changes during palette mode
            // ROOT CAUSE: CHROMA encoder shifts palette hue indices causing color corruption
            // SOLUTION: Block CHROMA updates when CONFIG.PALETTE_INDEX > 0 (palette mode)
            if (CONFIG.PALETTE_INDEX > 0) {
                if (DebugManager::should_print(DEBUG_COLOR)) {
                    USBSerial.printf("[E1] CHROMA change blocked in palette mode (req -> %.2f, keeping %.2f)\n",
                                     float(new_value), float(current_val));
                    DebugManager::mark_printed(DEBUG_COLOR);
                }
                return; // Block the change entirely
            }
    ...
    if (t_now - last_button_press_time > button_debounce_time) {
        ...
        if (encoder3_button_state && !encoder3_button_last_state) {
            ...
            if (t_now - encoder3_button_hold_start >= long_press_threshold) {
                ...
                if(debug_mode){
                    USBSerial.print("\033[36m[DBG E3]\033[0m Long Press | Contrast Mode: ");
                ...
                if (encoder3_in_contrast_mode) {
                    ...
                    settings_updated = true;
                    if(debug_mode){
                        USBSerial.print("\033[36m[DBG E3]\033[0m Short Press | Reset Contrast to: ");
                    ...
                    settings_updated = true;
                    if(debug_mode){
                        USBSerial.print("\033[36m[DBG E3]\033[0m Short Press | New Light Mode: ");
    ...
    // PALETTE BUTTON HANDLING [2025-09-20] - E0/E1/E2 palette control functions
    // E0: Palette up (only when CONFIG.PALETTE_INDEX > 0)
    // E1: Palette down (only when CONFIG.PALETTE_INDEX > 0)
    // E2: Toggle HSV (0) ↔ Palette mode (1+)

    auto handlePaletteButton = [&](uint8_t encoder_id, const char* action_name) {
        static uint32_t last_palette_button_press_time[3] = {0, 0, 0}; // E0, E1, E2
        static bool last_palette_button_state[3] = {false, false, false};
        static const uint32_t palette_button_debounce_time = 400; // Match E3 debounce

        uint8_t button_index = encoder_id; // 0, 1, 2 for E0, E1, E2
        if (button_index > 2) return; // Safety check

        if (t_now - last_palette_button_press_time[button_index] > palette_button_debounce_time) {
            bool current_button_state = rotate8.getKeyPressed(encoder_id);

            if (current_button_state && !last_palette_button_state[button_index]) {
                const uint8_t palette_total = palette_lut_count();
                const uint8_t max_palette_index = (palette_total > 0)
                                                  ? static_cast<uint8_t>(palette_total - 1)
                                                  : 0;

                // Button pressed - handle immediately for palette functions
                activity_detected = true;
                g_last_active_encoder = encoder_id;
                settings_updated = true;
                last_palette_button_press_time[button_index] = t_now;

                if (encoder_id == 0) {
                    // E0: Palette up (only when CONFIG.PALETTE_INDEX > 0)
                    if (CONFIG.PALETTE_INDEX > 0) {
                        CONFIG.PALETTE_INDEX++;
                        if (CONFIG.PALETTE_INDEX > max_palette_index) {
                            CONFIG.PALETTE_INDEX = (max_palette_index > 0) ? 1 : 0;
                        }
                        if (debug_mode) {
                            USBSerial.print("\033[36m[DBG E0 BTN]\033[0m Palette Up | PALETTE_INDEX=");
                    ...
                    if (CONFIG.PALETTE_INDEX > 0) {
                        CONFIG.PALETTE_INDEX--;
                        if (CONFIG.PALETTE_INDEX == 0 && max_palette_index > 0) {
                            CONFIG.PALETTE_INDEX = max_palette_index; // Wrap to last palette (skip HSV mode)
                        }
                        if (debug_mode) {
                            USBSerial.print("\033[36m[DBG E1 BTN]\033[0m Palette Down | PALETTE_INDEX=");
                            USBSerial.println(CONFIG.PALETTE_INDEX);
                        }
                    }
                    ...
                    // E2: Toggle HSV (0) ↔ Palette mode (1+)
                    if (CONFIG.PALETTE_INDEX == 0) {
                        CONFIG.PALETTE_INDEX = (max_palette_index > 0) ? 1 : 0; // Switch to first palette
                    } else {
                        CONFIG.PALETTE_INDEX = 0; // Switch to HSV mode
                    }
                    if (debug_mode) {
                        USBSerial.print("\033[36m[DBG E2 BTN]\033[0m Palette Toggle | PALETTE_INDEX=");
                        USBSerial.println(CONFIG.PALETTE_INDEX);
                    }
                }

                last_palette_button_state[button_index] = true;
            } else if (!current_button_state && last_palette_button_state[button_index]) {
                // Button released
                last_palette_button_state[button_index] = false;
            }
        }
    };

    // Process palette buttons E0, E1, E2
    handlePaletteButton(0, "Palette Up");
    handlePaletteButton(1, "Palette Down");
    handlePaletteButton(2, "Palette Toggle");
    ...
    if (encoder3_in_contrast_mode) {
        ...
        if (contrast_rel != 0) {
            ...
            if (new_value != current_val) {
                ...
                settings_updated = true;
                if(debug_mode) {
                    USBSerial.print("\033[36m[DBG E3 ROT]\033[0m Raw: "); USBSerial.print(contrast_rel);
