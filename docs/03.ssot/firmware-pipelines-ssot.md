...
- All timing assumes `CONFIG.SAMPLE_RATE = 16_000` Hz and `CONFIG.SAMPLES_PER_CHUNK = 128`
- CPU affinity: `primary_loop_task` runs alone on Core 0 (Phases A–E). LED rendering executes inside Phase C/D; `trace_consumer_task` is the only auxiliary low-priority task.
...
| Linear scaling & DC removal | `acquire_sample_chunk()` (`src/i2s_audio.h:79-132`) | `waveform[i]` & history | `short[CONFIG.SAMPLES_PER_CHUNK]` (±32767) | Clamped to ±32767 | `audio_processed_state`, `sample_window` | Uses `CONFIG.SENSITIVITY` (default 1.0) and `CONFIG.DC_OFFSET` (-14800). `MIN_STATE_DURATION_MS = 1500` governs sweet-spot transitions.
...
| Sliding window | `sample_window` update in `acquire_sample_chunk()` (`src/i2s_audio.h:209-248`) | `sample_window[SAMPLE_HISTORY_LENGTH]` | `short[4096]` | ±32767 | `process_GDFT()` | Window shift cost mitigated via `memmove`. Window length constant `SAMPLE_HISTORY_LENGTH = 4096` (32 chunks @ 16 kHz ≈ 256 ms context).
...
|-------|----------|--------|--------------|---------------|-----------|-------|
| Frame init & cache | `main_loop_core0()` Phase C (`src/main.cpp:360-430`) | `leds_16`, `leds_16_fx`, `leds_16_ui` | `CRGB16[NATIVE_RESOLUTION]` | 0.0–1.0 (Q8.8) | Mode renderers | `begin_frame()` clears buffers; `cache_frame_config()` snapshots CONFIG + Coupling Plan metadata.
...
| Mode dispatch (primary) | `main_loop_core0()` (`src/main.cpp:380-430`) | Mode-specific buffers | `CRGB16[]`, floats | 0–1 | LED output, debug overlay | Uses `frame_config.coordinator_primary_mode`; no CONFIG mutation.
| Mode dispatch (secondary) | `main_loop_core0()` (`src/main.cpp:430-520`) | Secondary buffer | `CRGB16[]` | 0–1 | LED output | Applies `frame_config` overrides (`coordinator_is_secondary`, hue detune) and calls `apply_antiphase_if_secondary()`; writes `leds_16_secondary`.
| GDFT mode (default) | `light_mode_gdft()` (`src/lightshow_modes.h:175-370`) | `leds_16_fx` spectral columns | `CRGB16[160]` | 0–1 | LED compositing | Consumes `spectrogram_smooth`, uses `frame_config.PHOTONS` brightness and note-based hue.
...
| Frame compositing | `show_leds()` (`src/main.cpp:520-560`, `src/led_utilities.h:928-1204`) | `leds_out` (FastLED) | `CRGB[CONFIG.LED_COUNT]` | 0–255 per channel | FastLED controller | Applies brightness scaling (`g_qos_brightness_scale`), dithering, incandescent/base coat guards, QoS symmetry checks.
| Output | `FastLED.show()` inside Phase D | Physical LEDs | WS2812 data stream | — | Hardware | Async double-buffer flip guarded by controller alignment checks.
...
| `leds_16` | `main_loop_core0()` | `show_leds`, Mabutrace traces |
...
| `frame_config.coordinator_*` | `cache_frame_config` (`src/lightshow_modes.cpp:8-30`) | Mode runners | Access Coupling Plan metadata without touching globals during render.
| `g_last_effective_prism_*` | `main_loop_core0()` (`src/main.cpp:430-505`) | QoS telemetry (serial/metrics) | Tracks symmetric prism trims for primary/secondary strips.
