...
static void handle_debug_audio_command(const char* args) {
  if (args == nullptr) {
    USBSerial.println("Usage: debug audio [on|off|snapshot|waveform|full|lite|interval <ms>]");
    return;
  }
  while (*args == ' ') {
    ++args;
  ...
  if (strcmp(args, "on") == 0 || strcmp(args, "true") == 0) {
    AudioDiagnostics::setEnabled(true);
    USBSerial.println("Audio diagnostics enabled");
    return;
  }
  if (strcmp(args, "off") == 0 || strcmp(args, "false") == 0) {
    AudioDiagnostics::setWaveformLogging(false);
    AudioDiagnostics::setEnabled(false);
    USBSerial.println("Audio diagnostics disabled");
    return;
  }
  if (strcmp(args, "snapshot") == 0) {
    AudioDiagnostics::requestSnapshot();
    USBSerial.println("Audio diagnostics snapshot requested");
    return;
  }
  if (strcmp(args, "full") == 0 || strcmp(args, "detailed") == 0) {
    AudioDiagnostics::setDetailed(true);
    USBSerial.println("Audio diagnostics set to detailed mode");
    return;
  }
  if (strcmp(args, "lite") == 0 || strcmp(args, "basic") == 0) {
    AudioDiagnostics::setDetailed(false);
    USBSerial.println("Audio diagnostics set to lite mode");
    return;
  }
  if (strncmp(args, "interval", 8) == 0) {
    ...
    if (*sub == 0) {
      USBSerial.print("Audio diagnostics interval: ");
      USBSerial.print(AudioDiagnostics::getIntervalMs());
      USBSerial.println(" ms");
      return;
    }
    uint32_t interval = static_cast<uint32_t>(max(0, atoi(sub)));
    AudioDiagnostics::setIntervalMs(interval);
    USBSerial.print("Audio diagnostics interval set to ");
    USBSerial.print(AudioDiagnostics::getIntervalMs());
    USBSerial.println(" ms");
    return;
  }
  if (strncmp(args, "waveform", 8) == 0) {
    const char* sub = args + 8;
    while (*sub == ' ') {
      ++sub;
    }
    if (*sub == 0 || strcmp(sub, "once") == 0) {
      AudioDiagnostics::requestSnapshot();
      USBSerial.println("Audio waveform snapshot requested");
      return;
    }
    if (strcmp(sub, "on") == 0) {
      AudioDiagnostics::setEnabled(true);
      AudioDiagnostics::setWaveformLogging(true);
      USBSerial.println("Audio waveform logging enabled");
      return;
    }
    if (strcmp(sub, "off") == 0) {
      AudioDiagnostics::setWaveformLogging(false);
      USBSerial.println("Audio waveform logging disabled");
      return;
    }
    USBSerial.println("Usage: debug audio waveform [on|off|once]");
    return;
  }
...
void parse_command(char* command_buf) {
  ...
  // Returns the number of modes available ------------------
    ...
    USBSerial.println(fxEngine.getNumEffects());
    tx_end();
  ...
  else if (strncmp(command_buf, "debug audio", 11) == 0) {
    const char* args = command_buf + 11;
    handle_debug_audio_command(args);
  }
  ...
  // COMMANDS WITH METADATA ##################################
    ...
    else if (strcmp(command_type, "debug_audio") == 0) {
      handle_debug_audio_command(command_data);
    }
    ...
    // Set Mode Number ----------------------------------------
      ...
      if (total == 0) {
        ...
        long requested = atol(command_data);
        if (requested < 0) requested = 0;
        if (requested >= total) requested = total - 1;

        const uint8_t dest = static_cast<uint8_t>(requested);
        CONFIG.LIGHTSHOW_MODE = dest;
        fxEngine.setEffect(dest, rt_transition_type, 800);
        settings_updated = true;
        save_config_delayed();

        tx_begin();
        USBSerial.print("CONFIG.LIGHTSHOW_MODE: ");
        USBSerial.println(dest);
        tx_end();
      }
    }
    ...
    // Get Mode Name By ID ------------------------------------
      ...
      if (mode_id < total) {
        const char* name = fxEngine.getEffectName(static_cast<uint8_t>(mode_id));
        tx_begin();
        USBSerial.print("MODE_NAME: ");
        USBSerial.println(name ? name : "unknown");
        tx_end();
      } else {
        bad_command(command_type, command_data);
      }
    ...
    // Secondary LED Controls ----------------------------
      ...
      USBSerial.print("SECONDARY_EFFECT: ");
      USBSerial.print(effect_index);
      USBSerial.print(" (");
      USBSerial.print(effect_name ? effect_name : "unknown");
      USBSerial.println(")");
      USBSerial.print("SECONDARY_PHOTONS: ");
      USBSerial.println(SECONDARY_PHOTONS, 6);
      USBSerial.print("SECONDARY_CHROMA: ");
    ...
    // Add backward compatibility for old commands
      ...
      USBSerial.println("Secondary render now mirrors the primary LC effect. Command ignored.");
    }
