...
void M5UnitScroll::resetEncoder(void) {
    uint8_t data = 1;
    writeBytes(_addr, RESET_REG, &data, 1);
}
...
uint8_t M5UnitScroll::getBootloaderVersion(void) {
    uint8_t data;
    readBytes(_addr, BOOTLOADER_VERSION_REG, &data, 1);
    return data;
}

uint8_t M5UnitScroll::getFirmwareVersion(void) {
    uint8_t data;
    readBytes(_addr, FIRMWARE_VERSION_REG, &data, 1);
    return data;
}

uint8_t M5UnitScroll::setI2CAddress(uint8_t addr) {
    _addr = addr;
    writeBytes(_addr, I2C_ADDRESS_REG, &_addr, 1);
    return _addr;
}

uint8_t M5UnitScroll::getI2CAddress(void) {
    uint8_t data;
    readBytes(_addr, I2C_ADDRESS_REG, &data, 1);
    return data;
}
