...
inline uint8_t byte01(SQ15x16 v) {
...
inline CRGB16 hsv_or_palette(SQ15x16 hue01, SQ15x16 sat01, SQ15x16 val01) {
  if (CONFIG.PALETTE_INDEX == 0) {
    return hsv(hue01, sat01, val01);
  }

  const uint8_t palPos = CONFIG.AUTO_COLOR_SHIFT ? 128 : byte01(hue01);
  const CRGB sampled = ColorFromPalette(currentPalette, palPos, 255, LINEARBLEND);

  CRGB16 result;
  result.r = SQ15x16(sampled.r / 255.0f);
  result.g = SQ15x16(sampled.g / 255.0f);
  result.b = SQ15x16(sampled.b / 255.0f);
...
inline void show_leds() {
  ...
  constexpr uint32_t kShowLogIntervalMs = 1000;  // milliseconds between log lines when idle
  ...
  if (debug_mode && (millis() % 5000 == 0)) {
    ...
    const uint8_t effect_index = (total == 0) ? 0 : (CONFIG.LIGHTSHOW_MODE % total);
    const char* effect_name = (total == 0) ? "n/a" : fxEngine.getEffectName(effect_index);
    USBSerial.printf("DEBUG: Active LC effect -> %u (%s)\n", effect_index, effect_name ? effect_name : "unknown");
  }
}
...
inline void init_leds() {
  ...
  if (leds_scaled == nullptr || leds_out == nullptr) {
    ...
    SB_RESTART("init_leds primary alloc failed");
  }
  ...
  if (ENABLE_SECONDARY_LEDS) {
    ...
    if (leds_scaled_secondary == nullptr || leds_out_secondary == nullptr) {
      ...
      SB_RESTART("init_leds secondary alloc failed");
    }
  ...
  if (CONFIG.LED_TYPE == LED_NEOPIXEL) {
    if (CONFIG.LED_COLOR_ORDER == RGB) {
      FastLED.addLeds<WS2812B, HardwareConfig::LED_DATA_PIN, RGB>(leds_out, CONFIG.LED_COUNT);
    } else if (CONFIG.LED_COLOR_ORDER == GRB) {
      FastLED.addLeds<WS2812B, HardwareConfig::LED_DATA_PIN, GRB>(leds_out, CONFIG.LED_COUNT);
    } else if (CONFIG.LED_COLOR_ORDER == BGR) {
      FastLED.addLeds<WS2812B, HardwareConfig::LED_DATA_PIN, BGR>(leds_out, CONFIG.LED_COUNT);
    }
  }

  else if (CONFIG.LED_TYPE == LED_NEOPIXEL_X2) {
    if (CONFIG.LED_COLOR_ORDER == RGB) {
      FastLED.addLeds< WS2812B, HardwareConfig::LED_DATA_PIN,  RGB >(leds_out, 0, CONFIG.LED_COUNT / 2);
      FastLED.addLeds< WS2812B, HardwareConfig::SECONDARY_LED_DATA_PIN, RGB >(leds_out, CONFIG.LED_COUNT / 2, CONFIG.LED_COUNT / 2);
    } else if (CONFIG.LED_COLOR_ORDER == GRB) {
      FastLED.addLeds< WS2812B, HardwareConfig::LED_DATA_PIN,  GRB >(leds_out, 0, CONFIG.LED_COUNT / 2);
      FastLED.addLeds< WS2812B, HardwareConfig::SECONDARY_LED_DATA_PIN, GRB >(leds_out, CONFIG.LED_COUNT / 2, CONFIG.LED_COUNT / 2);
    } else if (CONFIG.LED_COLOR_ORDER == BGR) {
      FastLED.addLeds< WS2812B, HardwareConfig::LED_DATA_PIN,  BGR >(leds_out, 0, CONFIG.LED_COUNT / 2);
      FastLED.addLeds< WS2812B, HardwareConfig::SECONDARY_LED_DATA_PIN, BGR >(leds_out, CONFIG.LED_COUNT / 2, CONFIG.LED_COUNT / 2);
    }
  }

  else if (CONFIG.LED_TYPE == LED_DOTSTAR) {
    // NOTE: The secondary channel is treated as the APA102 clock line when DOTSTAR mode is active.
    if (CONFIG.LED_COLOR_ORDER == RGB) {
      FastLED.addLeds<DOTSTAR, HardwareConfig::LED_DATA_PIN, HardwareConfig::SECONDARY_LED_DATA_PIN, RGB>(leds_out, CONFIG.LED_COUNT);
    } else if (CONFIG.LED_COLOR_ORDER == GRB) {
      FastLED.addLeds<DOTSTAR, HardwareConfig::LED_DATA_PIN, HardwareConfig::SECONDARY_LED_DATA_PIN, GRB>(leds_out, CONFIG.LED_COUNT);
    } else if (CONFIG.LED_COLOR_ORDER == BGR) {
      FastLED.addLeds<DOTSTAR, HardwareConfig::LED_DATA_PIN, HardwareConfig::SECONDARY_LED_DATA_PIN, BGR>(leds_out, CONFIG.LED_COUNT);
    }
  }
    ...
    if (mode_transition_queued == true) {  // If transition for MODE button press
      ...
      if (total > 0) {
        if (mode_destination == -1) {  // Triggered via button
          CONFIG.LIGHTSHOW_MODE = (CONFIG.LIGHTSHOW_MODE + 1) % total;
        } else {  // Triggered via Serial
          if (mode_destination < 0) mode_destination = 0;
          if (mode_destination >= total) mode_destination = total - 1;
          CONFIG.LIGHTSHOW_MODE = static_cast<uint8_t>(mode_destination);
          mode_destination = -1;
        }
        fxEngine.setEffect(CONFIG.LIGHTSHOW_MODE, rt_transition_type, 800);
...
inline void init_secondary_leds() {
  ...
  FastLED.addLeds<WS2812B, HardwareConfig::SECONDARY_LED_DATA_PIN, GRB>(leds_out_secondary, SECONDARY_LED_COUNT_CONST);
