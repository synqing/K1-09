  ...
  for (;;) {
    while (g_trace_buffer.pop(event)) {
      if (g_trace_config.enable_serial) {
        // Minimal, low-overhead print. Use ets_printf to avoid USB stack stalls.
        ets_printf("[TRACE] t=%u id=0x%04X core=%u lvl=%u data=0x%08X\r\n",
                   event.timestamp, event.event_id, event.core_id, event.level, event.data);
      }
      // TODO: add WiFi/SD exporters when enabled
    }
    vTaskDelay(1);
  }
}
